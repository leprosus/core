...
final class DB {
    ...
    public function query($query, $arguments = array()) {
        ...
        try {
            $hash = md5($query);
            $statement = isset($this->cache[$hash]) ? $this->cache[$hash] : ($this->cache[$hash] = $this->db->prepare($query));
     ...
     * @throws \Core\Exception\InvalidArgument
     * @throws \Core\Exception\SQL
     * @return array|null Ассоциированный массив
     */
    ...
    public function queryAndFetchAll($query, $arguments = array()) {
        ...
        if(preg_match('#^\s*SELECT#i', $query)) {
            ...
            try {
                ...
                $this->lastQuery = self::prepare($statement, $arguments);
                $statement->execute();
                $result = ($fetch = $statement->fetchAll()) ? $fetch : array();
            } catch(\PDOException $error) {
                throw new Exception\SQL('Incorrect query ' . $this->lastQuery . "\n" . $error->getMessage());
            }
        }
        ...
        if(count($arguments) > 0) {
            foreach($arguments as $key => $value) {
                if(preg_match('#^:[a-z]+#', $key)) {
                    $statement->bindValue($key, $value, Type::isNull($value) ? \PDO::PARAM_NULL : (Type::isInteger($value) ? \PDO::PARAM_INT : \PDO::PARAM_STR));
                    $query = str_replace($key, (Type::isNull($value) ? 'NULL' : (Type::isInteger($value) ? $value : '\'' . $value . '\'')), $query);
                    unset($arguments[$key]);
                }
            }
            ...
            foreach($arguments as $key => $value) {
                $statement->bindValue($key + 1, $value, Type::isNull($value) ? \PDO::PARAM_NULL : (Type::isInteger($value) ? \PDO::PARAM_INT : \PDO::PARAM_STR));
                if(($position = \mb_strpos($query, '?', 0, 'UTF-8')) !== false) {
                    $query
                        = mb_substr($query, 0, $position, 'UTF-8') . (Type::isNull($value) ? 'NULL' : (Type::isInteger($value) ? $value : '\'' . $value . '\'')) . mb_substr($query,
                                                                                                                                                                             $position
                                                                                                                                                                             + 1,
                                                                                                                                                                             mb_strlen($query,
                                                                                                                                                                                       'UTF-8'),
    ...
    private static function prepareArgs($argumetns) {
        foreach($argumetns as $key => &$value) {
            if(Type::isArray($value)) {
                ...
                $value = preg_match('#^(\d{4}-\d{2}-\d{2}(?:\s\d{2}:\d{2}(?::\d{2})?)?|\d{2}:\d{2}(?::\d{2})?)$#', $value) ? $value : Filter::screen($value);
            } else {
                unset($argumetns[$key]);
            }
        }
