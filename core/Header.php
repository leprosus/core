...
final class Header {
    ...
    public function setXFrameOptions($mode) {
        return preg_match('#^(deny|sameorigin|allow\-from(\shttp(s?)\://[\da-z][\-\._\da-z]+\.[a-z]{2,6})?)$#i', $mode) ? $this->addHeader('X-Frame-Options', $mode) : $this;
    }
    ...
    public static function sendStatus($code = 200) {
        ...
        if(isset($list[$code])) {
            $protocol = Server::getInstance()
                              ->getParam('SERVER_PROTOCOL', '#^HTTP/1\.[01]$#', 'HTTP/1.0');
            $text = $code . ' ' . $list[$code];
     ...
     * @return void
     */
    public static function sendLocation($link, $isPermanent = false) {
        \Core\Cookie::getInstance()
                    ->send();
