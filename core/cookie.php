...
final class Cookie{
    ...
    public function add($name, $value, $expire = 0, $path = '/', $domain = '', $secure = false, $httponly = false){
        if(Server::getInstance()->getParam('HTTP_HOST') == 'localhost'){
            $path = '/';
            $domain = false;
        }
    ...
    public function send(){
        foreach($this->data as $current){
            ...
            header('Set-Cookie: '.rawurlencode($name).'='.rawurlencode($value).(empty($expire) ? ''
                : '; expires='.gmdate('D, d-M-Y H:i:s', $expire).' GMT').(empty($path) ? ''
                : '; path='.$path).(empty($domain) ? '' : '; domain='.$domain).($secure ? '; secure' : '').($httponly
                ? '; HttpOnly' : ''), false);
        }
    }
