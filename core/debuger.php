...
final class Debuger{
    ...
    public function run(){
        ...
        set_error_handler(function ($errNumber, $errString, $errFile, $errLine) use ($self, $log, $mail){
            ...
            $levelMapping = array(
                \E_ERROR => \Core\Log::LEVEL_ERROR, \E_USER_ERROR => \Core\Log::LEVEL_ERROR,
                \E_WARNING => \Core\Log::LEVEL_WARN, \E_USER_WARNING => \Core\Log::LEVEL_WARN,
                \E_NOTICE => \Core\Log::LEVEL_WARN, \E_USER_NOTICE => \Core\Log::LEVEL_WARN
            );

            $errorTypes = array(
                \E_ERROR => 'ERROR', \E_USER_ERROR => 'ERROR', \E_WARNING => 'WARN', \E_USER_WARNING => 'WARN',
            ...
            if(in_array($errNumber, array(E_ERROR, E_USER_ERROR, E_WARNING, E_USER_WARNING, E_NOTICE, E_USER_NOTICE))){
                ...
                $data[] = $errorTypes[$errNumber].': ['.$errNumber.'] '.$errString."\n".'Error on line '.$errLine.' in file '.$errFile;

                if(mb_strlen(\Core\Router::getInstance()->getQuery(), 'UTF-8') > 0
                ){
                    $data[] = 'URL:'.\Core\Router::getInstance()->getQuery();
                }

                if(count($post = \Core\Request::getInstance()->getParams()) > 0
                ){
                    $data[] = 'POST:'."\n".var_export($post, true);
                }

                if(count($server = \Core\Server::getInstance()->getParams()) > 0
                ){
                    $data[] = 'SERVER:'."\n".var_export($server, true);
                }

                if(!is_null($ip = \Core\Server::getInstance()->getIPAddress())
                ){
                    $data[] = 'IP:'."\n".var_export($ip, true);
                }
        ...
        set_exception_handler(function (\Exception $exception) use ($self, $log, $mail){
            ...
            $level = $isInternal && isset($levelMapping[$exception->getLevel()]) ? $levelMapping[$exception->getLevel()]
                : \Core\Log::LEVEL_ERROR;

            do{
                $trace = $exception->getTrace();
                foreach($trace as $key => &$value){
                    $value = "\t".sprintf('#%s %s: %s(%s)', $key, isset($value['file'])
                        ? $value['file'].'('.$value['line'].')'
                        : '{closure}', $value['function'], implode(', ', array_map('gettype', $value['args'])));
                }

                $reflection = new \ReflectionClass($exception);

                $data[] = sprintf('Uncaught %sexception \'%s\' with message \'%s\' in %s: %s'."\n".'Stack trace:'."\n".'%s', $reflection->isSubclassOf('\\Core\\Exception\\Simple')
                    ? $exception->getLevel().' '
