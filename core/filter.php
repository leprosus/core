 ...
 */
final class Filter{
    private static $badFileNameCharacters = array(
        '<!--', '-->', '<', '>', '\'', '"', '&', '$', '#', '{', '}', '[', ']', '=', ';', '?', '%20', '%22', '%3c',
    ...
    public static function clearXSS($content){
        if(is_string($content)){
            ...
            $content = preg_replace('#<.*?(?:href|src|style)\s*=.*?(alert|javascript\s*\:|livescript\s*\:|mocha\s*\:|charset\s*\=|window\s*\.|document\s*\.|expression\s*(\(|&\#40;)|\.\s*cookie|<\s*script|<\s*xss|base64\s*,).*?>#iu', '', $content);

            $patterns = array(
                '#document\s*.\s*cookie#iu' => '', '#document\s*.\s*write#iu' => '', '#.\s*parentNode#iu' => '',
                '#.\s*innerHTML#iu' => '', '#window\s*.\s*location#iu' => '', '#-moz-binding#iu' => '',
                '#<!--#iu' => '&lt;!--', '#-->#iu' => '--&gt;', '#<!\[CDATA\[#iu' => '&lt;![CDATA[',
                '#<comment>#iu' => '&lt;comment&gt;', '#javascript\s*:#iu' => '', '#expression\s*(\(|&\#40;)#iu' => '',
                '#vbscript\s*:#iu' => '', '#Redirect\s+302#iu' => ''
            );
