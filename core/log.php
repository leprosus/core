...
final class Log extends Common{
    ...
    private $priority = array(
        self::LEVEL_FATAL => 1, self::LEVEL_ERROR => 3, self::LEVEL_WARN => 7, self::LEVEL_DEBUG => 15,
        self::LEVEL_INFO => 31, self::LEVEL_ALL => 63
    );
    ...
    public function write($message, $level = self::LEVEL_INFO){
        if(preg_match('#^(DEBUG|INFO|WARN|ERROR|FATAL)$#', $level) && ($this->priority[$this->level] | $this->priority[$level]) == $this->priority[$this->level]){
