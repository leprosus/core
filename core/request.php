...
final class Request extends Common{
    ...
    public function getParam($name, $regexp = null, $default = null){
        return $this->hasParam($name, $regexp) ? (is_null($regexp) ? Filter::screen($this->params[$name])
    ...
    public function hasFile($name, $extensionRegexp = null, $nameRegexp = null, $maxSize = null){
        return isset($this->files[$name]) && $this->files[$name]['error'] == 0 && (is_null($extensionRegexp) || preg_match($extensionRegexp, $this->files[$name]['extension'])) && (is_null($nameRegexp) || preg_match($nameRegexp, $this->files[$name]['name'])) && (is_null($maxSize) || $this->files[$name]['size'] <= $maxSize);
    }
    ...
    public function getFileErrorMessage($name){
        $errors = array(
            0 => 'The file uploaded with success', 1 => 'The file size is too big (see php.ini)',
            2 => 'The file size is too big (see MAX_FILE_SIZE in HTML form)', 3 => 'The file was partially uploaded',
            4 => 'No file was uploaded', 6 => 'Missing a temporary folder (see php.ini)',
