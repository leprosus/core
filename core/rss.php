...
final class Rss{
    ...
    public function generateRss(){
        $rsstext = '<?xml version="1.0"?><rss version="2.0"><channel>'.'<title>{$this->name}</title>'.'<link>{$this->link}</link>'.'<description>{$this->description}</description>'.'<language>{$this->language}</language>'.'<lastBuildDate>{$this->date}</lastBuildDate>';
        foreach($this->items as $current){
            list($title, $link, $description, $date) = $current;
            $rsstext .= '<item>'.'<title>{$title}</title>'.'<link>{$link}</link>'.'<description>{$description}</description>'.'<pubDate>{$date}</pubDate>'.'</item>';
        }
        $rsstext .= '</channel></rss>';
